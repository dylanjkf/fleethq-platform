...
password reset (`mustResetPassword`, set on bootstrap-created accounts), or —
because staff MFA is required by default (`ENFORCE_STAFF_ADMIN_MFA`, fail closed;
disableable only outside production) — MFA not yet enrolled — is blocked by
