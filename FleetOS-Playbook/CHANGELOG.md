...
- **C2 — Watched end-to-end deploy.** The CI-gated Railway pipeline (`deploy-api.yml` on `api-ci` success) was proven working in Round 4-prior by two watched runs (#7 `d854e9f`, #8 `ff289df`, both `conclusion: success`). **Not re-verified this round end-to-end:** the R4 code sits on the `claude/…` feature branch, and a fresh production deploy only runs once it reaches `main` (api-ci → deploy-api), which is a merge I have not performed. I did **not** trigger a `workflow_dispatch` of the current `main`, because that would deploy old code, not this work. Honest status: pipeline verified; a deploy **of the R4 changes** is pending merge to `main`.
