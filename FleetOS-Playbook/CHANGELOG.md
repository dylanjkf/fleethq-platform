...
- **H1 — Staff MFA secure-by-default: deliberately NOT restored.** The audit asked to restore forced staff MFA; the **founder explicitly chose to keep MFA optional (default OFF)** — `staffAdminMfaEnforced()` stays opt-in via `ENFORCE_STAFF_ADMIN_MFA=true`. Recorded as a product decision, not an oversight; it caps the Security score by design.
