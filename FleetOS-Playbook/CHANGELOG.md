...
- **C1 — Deploy recency guard was computed but not wired (fixed, fail-closed).** `deploy-api.yml`'s "Recency guard" step correctly set `SUPERSEDED=1` when a commit had been overtaken on `main`, but only the harmless *Preflight* step checked that flag — "Install Railway CLI" and "Deploy api/ to Railway" had **no `if:`**, so a stale, superseded commit still ran `railway up` and could overwrite a newer deploy. Directly contradicted R4's own "stands down instead of shipping stale code" claim. **Fix, in two layers:** (1) the guard step is now **fail-closed** — on supersession it sets `SUPERSEDED=1` *and* `exit 1`, so the job halts there and *no* later step runs, meaning forgetting an `if:` on any future step can't silently reopen the race; (2) belt-and-suspenders `if: ${{ env.SUPERSEDED != '1' }}` added to *Install*, *Deploy*, and the new smoke-test step. **Verification — proven with a real forced-supersession run, not a YAML re-read** (full log excerpts + run URLs saved durably at `FleetOS-Playbook/evidence/R5-C1-supersession-proof.md`): two `api/`-touching commits were merged back-to-back — A `ce769db` (older), B `4d0674a` (newer, became `main` tip). When A's `api-ci` finished and triggered its deploy, the guard printed `Deploying: ce769db… / main tip: 4d0674a…`, set `SUPERSEDED=1`, logged `::error::main has already advanced past…`, and `exit 1` — the job went straight to *Post job cleanup*, so **"Install Railway CLI", "Deploy api/ to Railway", and the smoke test did NOT execute** (`railway up` never ran for the stale commit): run [30859124698](https://github.com/dylanjkf/fleethq-platform/actions/runs/30859124698), conclusion **failure** at the guard. The R5 merge commit `b6c0c3bb` was superseded the same way ([30859045378](https://github.com/dylanjkf/fleethq-platform/actions/runs/30859045378)). Meanwhile the newest commit B deployed normally — guard proceeded, `railway up` succeeded, smoke test ran: run [30859127650](https://github.com/dylanjkf/fleethq-platform/actions/runs/30859127650), **success**. Exactly one deploy shipped (the newest); the two stale ones were refused. The race is enforced, not merely detected.

  **R7 re-verification (2026-08-03) — supersession re-proven; two dependent settings found NOT in effect, reported not claimed.** Re-ran the forced-supersession probe after a repo-admin was expected to have enabled branch protection and set `API_HEALTH_URL`. The guard itself re-confirmed working: older `b64e4b86` stood down — run [30860177529](https://github.com/dylanjkf/fleethq-platform/actions/runs/30860177529), log `Deploying: b64e4b86… / main tip: 68809eb6…`, `SUPERSEDED`, `::error…standing down`, `exit 1`, and the job went straight to *Post job cleanup* (Install/Deploy/smoke never ran); newer `68809eb6` deployed normally — run [30860176387](https://github.com/dylanjkf/fleethq-platform/actions/runs/30860176387), `Deploy complete`. **But the two settings the R7 brief said were already applied are NOT in effect in the pipeline, so the two verifications depending on them could not be captured — reporting the gap rather than asserting it closed:** (a) **`API_HEALTH_URL` is unset at runtime** — run 30860176387's smoke-test step logged `env: API_HEALTH_URL:` (blank) then `::warning::API_HEALTH_URL not set — post-deploy smoke test skipped`, i.e. it warn-and-passed again and did **not** hit `/health/ready`; the self-verifying deploy is still inactive. Likely cause: it was added as a *Secret* or an *Environment*-scoped variable, or under a different name/repo — the deploy job reads `vars.API_HEALTH_URL` at repository scope. (b) **Branch protection on `main` is not active** — `branches/main.protected == false`, `GET /repos/…/rules/branches/main` returns `[]`, and probe PRs #31/#32 reported `mergeable_state: "unstable"` (not `"blocked"`) and merged with no passing required check and no review. Likely cause: a ruleset left in *Evaluate*/*Disabled* enforcement, a branch-name target that doesn't match `main`, or configuration on a different repo. **Both remain UNVERIFIED until those settings actually take effect.**
- **C2 — `reconcileAdminPermissions` had no empty-input guard and would mass-delete (fixed + tested).** `admin-permissions-reconcile.ts` computes "retired" permissions as everything in the DB not in `ADMIN_PERMISSION_CATALOG`, then `deleteMany`s them — cascading through the `AdminRolePermission` FK to strip **every** role's grants, including future custom roles. If that catalog import were ever empty (broken build / bad refactor), one call would wipe the entire admin RBAC surface. `prod-bootstrap.ts` already guards the customer `PERMISSION_CATALOG` this exact way; the admin path did not. **Fix:** added the identical fail-closed guard (`!Array.isArray || length === 0 → throw`) at the very top, before anything is computed or deleted. **Verification:** new `admin-permissions-reconcile.spec.ts` with two tests, both green locally — (1) an empty catalog **throws and `deleteMany` is never called** (asserted on the mock); (2) with the real catalog the normal retirement path still works — a key genuinely dropped from the catalog is deleted while a still-present key is untouched. Also extracted the deletion into a `retireStalePermissions` helper (keeps the guarded function under the line limit; no behaviour change).

**High**

- **H4 — "Deploy verified" claims are now self-verifying (implemented).** Added a **post-deploy smoke-test step** to `deploy-api.yml`: after `railway up` returns, it polls the live `/health/ready` endpoint (which pings Postgres) for up to ~3 min and **fails the job** unless it gets `HTTP 200` + `"status":"ok"`. The workflow's own pass/fail becomes the evidence, replacing hand-written run-ID narratives. **Honest boundary:** activation needs a one-time repo **Variable** `API_HEALTH_URL` (the API's public `/health/ready` URL) that I cannot set from here (no repo-settings API in the available tooling); until it is set the step emits a visible `::warning::` and passes rather than hard-failing on unconfigured infra — exactly like the existing `RAILWAY_*` preflight. So: the mechanism is in place and wired; whether it is *active* depends on that variable being set, which the changelog does not claim to have done. **Observed running:** in the newest-commit deploy [30859127650](https://github.com/dylanjkf/fleethq-platform/actions/runs/30859127650) the "Post-deploy smoke test" step executed after `railway up` and passed via the warn-and-pass path (variable unset) — confirming it is wired into the real pipeline and will hard-fail once `API_HEALTH_URL` is set.
- **H3 — Branch protection on `main` still off — NOT fixed from here, stated plainly.** Confirmed again via the GitHub API this round (`protected: false`). This is a repo-**Settings** change (Require status checks = `api-ci`; Require review from Code Owners) with **no API exposed in this environment's GitHub tooling** to set it — so I could not enable it, and I am not claiming to have. **Action required by a repo admin:** Settings → Branches → Add rule for `main` → enable "Require status checks to pass" (select `api-ci`) and "Require a pull request before merging" + "Require review from Code Owners" (a root `CODEOWNERS` already exists). Re-check with `GET /repos/dylanjkf/fleethq-platform/branches/main` and confirm `"protected": true` with the `api-ci` check listed.



Fourth independent audit treated as source of truth. Two process mandates applied throughout: (1) for any *pattern* fix, grep the whole tree myself rather than trusting a prior file list; (2) for anything involving a deployed artifact, verify against the running system and **say so explicitly in the changelog when I could not verify end-to-end** rather than asserting it's done.

**Critical**

- **C1 — Documentation fabricated-infrastructure, made exhaustive + a real CI gate.** Replaced the weak "mentions `infra/terraform` + a banner somewhere in the file" guard (which let a top-of-file banner "qualify" a false claim 100 lines below) with a **proximity-scoped** checker, `scripts/check-doc-integrity.py`: a present-tense AWS/Terraform infra claim (RDS, ECS/Fargate, ALB, VPC, KMS, SSE-KMS, CloudFront, Secrets Manager, ECR, Route53, DynamoDB, `infra/terraform`, `ap-southeast`, auto-scaling, PITR, …) is a violation unless a planned/target qualifier sits **within 5 lines** of it. Swept the whole tree (`docs/` + `FleetOS-Playbook/` + `README.md`, plus a raw superset grep across every `*.md`) and corrected or adjacently-qualified **all 29** hits — Production_Operations, migrations-backup-scaling, database/assessment, cyber-essentials 02/04/06, backup-and-disaster-recovery, Codebase_Audit, Scaling_And_Enterprise_Readiness, Integration_Hub (the real AES-256-GCM vault, made precise), Security_Review, Go_Live_Runbook, Launch_Readiness_Plan, Billing_And_Subscriptions, DEPLOYMENT_AND_GO_TO_MARKET, Privacy_Policy, statement-of-applicability, 20-Legal/README. The checker has a built-in `--selftest` that runs first in CI, so the guard can't silently rot into a no-op. **Verified:** `--selftest` green; a throwaway fabricated claim (`docs/_redteam_fabricated.md`) made the checker exit 1, then passed once removed; full scan reports 0. `docs-integrity.yml` rewritten to run the checker + self-test.

  **Second pass (after re-reading the brief) — the "narrow named-list" failure, demonstrated and closed.** A raw independent grep (not trusting the checker) found **5 more** present-tense existence fabrications that both a distant qualifier *and* my first checker had excused, none on the brief's named list: `Codebase_Audit_2026-07.md`'s "Terraform modules under `infra/` (ECS/RDS/…)" deployment paragraph, `Launch_Readiness_Plan.md`'s "the Terraform … are authored" framing, `Security_Review.md`'s "Secrets Manager in production"/"CloudFront … in production" parentheticals, `Billing_And_Subscriptions.md`'s "Terraform's Secrets Manager module already has…", and `05-malware…md`'s citation of specific line numbers in a non-existent `.tf`. All corrected to reality. The gate was then hardened so those *couldn't* have passed: proximity tightened 5→3 lines; a `(planned)` **section header** now governs its bullets (so honest planned sections don't false-positive) **except** a bullet that itself asserts it's live (`in production`/`already provisioned`), which is only excused by a qualifier in the *same wrapped sentence* — never by a distant `planned` word or header; markdown emphasis (`**no** Terraform`) no longer hides an honest negation; and the weak `deferred`/`until` qualifiers (which had let "deployment is deferred until X" excuse a Terraform *existence* claim) were removed. Now 10 self-test cases including two red-team scenarios (a fabricated claim under a `## Planned target` header still fails the build) and honest-reality cases (a "current deployment" line under a planned header still passes).
- **C2 — Watched end-to-end deploy — now verified for the R4 changes.** The R4 work was merged to `main` (squash `68e2588`, PR #24). That first pipeline run caught a **real regression this very changelog process is meant to catch**: `api-ci` **failed** (run `30804792201`) because a new R4 unit spec (`admin-setup-obligations.spec.ts`) stubbed a function via `jest.spyOn(require(...))`, tripping `@typescript-eslint/no-require-imports` — one lint error, exit 1. Because `deploy-api` gates on `api-ci` **success**, it correctly **skipped** (run `30804850209`) rather than shipping unvalidated code — the C2 gate doing exactly its job. Fixed the spec (namespace import `import * as adminRolePermissions`, drop unused `ForbiddenException`; PR #25, squash `8883e146`); re-ran the full pipeline and **watched it end-to-end**: `api-ci` **success** (run `30805525087` — lint clean, build, and the whole e2e suite incl. the `pg_trgm` GIN-index migration against CI's real Postgres) → `deploy-api` **success** (run `30805778607`): recency-guard passed, `railway up` ran a real Docker multi-stage build (`npm ci` → `nest build` → non-root runtime image), pushed image `sha256:927966b3…`, and logged **`Deploy complete`**. Honest status: **pipeline and a production deploy of the R4 changes both verified end-to-end** (not a `workflow_dispatch`; the real merge-driven `api-ci → deploy-api` chain).
- **C3 — Live "encrypted at rest" homepage claim.** The marketing-site **source** is clean: every security claim reads "encrypted **in transit**" and there is no "at rest" assertion anywhere (`Compliance.tsx`, `security/page.tsx`, `content/platform.ts`, `content/social-proof.ts`), and the corrected version is on `fleethqwebsite` `main` (PR #13, merged). **Could not verify the live render end-to-end:** the sandbox's egress proxy blocks `fleethq.net.au` (confirmed: `curl … 403 CONNECT tunnel failed`). If the live page still shows "at rest", it is a stale Vercel build serving an old bundle, not a source defect — it needs a website redeploy from `main` and a manual/user check I cannot perform from here.

**High**

- **H1 — Staff MFA secure-by-default: RESTORED (this entry's earlier claim was wrong).** A prior round recorded staff MFA as a deliberate founder decision to keep it "optional (default OFF)". That was not a sanctioned product decision — it was a fail-open regression that shipped when the customer-side "MFA optional after signup" change also flipped the unrelated staff/admin default. Staff/admin accounts run on the BYPASSRLS role with cross-tenant reach, so the default is now restored to **fail closed**: `staffAdminMfaEnforced()` returns `true` unless an explicit `ENFORCE_STAFF_ADMIN_MFA=false` opt-out is set, and that opt-out is ignored in production. The `admin-setup-obligations.spec.ts` truth table now guards the default so this cannot silently regress the same way again.
- **H2 — Forced rotation on the normal admin onboarding path.** `admin-users.service.create()` now sets `mustResetPassword: true`, so a console-onboarded staff account must rotate its creator-chosen temporary password on first login (the `ADMIN_SETUP_REQUIRED` gate blocks every route until it does) — previously only the bootstrap path forced this. Spec asserts it.
- **H3 — Obligations-gate + MFA-policy coverage.** New runnable unit spec (`admin-setup-obligations.spec.ts`) exercises the exact `403 ADMIN_SETUP_REQUIRED` code path (password-reset and MFA-enrolment obligations, setup-exempt bypass, MFA-optional pass-through) and `staffAdminMfaEnforced()` truth table. DB-backed e2e runs in api-ci.
- **H4 — DriverOS 401 session wipe now awaited.** `setUnauthorizedHandler` accepts an async handler, `client.ts` awaits it, and the handler awaits `purgeSessionData()` instead of `void`-ing it — the IndexedDB + push teardown is guaranteed to finish, matching the explicit-logout path.
- **H5 — Branch protection on `main`.** **Verified via the GitHub API: `main` is `"protected": false` — branch protection is NOT enabled.** I cannot set it via the available tooling (no branch-protection endpoint; it needs repo-admin action in GitHub → Settings → Branches). Documented as a required manual step in the runbook and security policy; not claimed as done.
- **H6 — Out-of-order deploy race.** `deploy-api.yml` gains a recency guard: if `main` has already advanced past the commit whose CI just succeeded (a slow older push), the deploy stands down instead of shipping stale code over newer.
- **H7 — Admin nav-drawer focus trap.** The mobile drawer now uses `useModalDialog` (focus-in on open, Tab trapped, Escape closes, focus restored) — extracted to a `MobileNavDrawer` component so the hook mounts with the drawer.
- **H8 — `text-danger-400` undefined in theme.** Added the missing lighter `--color-danger-400` token (danger-500 at L≈0.58 is too dark for body text on the admin's dark surfaces to clear AA); CommandPalette + ErrorState error copy now use it. danger-500 stays for fills/borders.
- **H9 — Admin global-search trigram index.** Migration `20260803020000` adds `pg_trgm` GIN indexes for the leading-wildcard `ILIKE` search over `companies.name`, `users.full_name/email/username`, `assets.name/registration/vin` — was a sequential scan per keystroke. (Applied + verified in api-ci's real-Postgres run, not in this sandbox.)
- **H10 — Push unsubscribe before token clear.** DriverOS `logout` now tears down the push subscription **first** (while the token is valid, so `POST /v1/push/unsubscribe` is authenticated and actually removes it), then clears the token, then wipes local caches — previously the token was cleared first, leaving the subscription live server-side so the next driver's device could keep receiving this driver's notifications. New `AuthProvider.spec.tsx` asserts the ordering and the awaited 401 purge.

**Medium**

- Bootstrap fails loud if `PERMISSION_CATALOG` is empty (a broken build would otherwise silently seed 0 permissions and open the RBAC surface). `reconcileAdminPermissions` now **deletes** retired (out-of-catalog) permission rows (grants cascade). The `ADMIN_SETUP_REQUIRED` denial is now written to the admin audit log.
- Go_Live_Runbook documents the CI-gated deploy model; `.railwayignore` excludes `/.git` and `/scripts` from the build context.
- **MFA UX:** new `POST /v1/admin/auth/mfa/backup-codes/regenerate` (verify a current factor → replace the hashed set → audit; unit-tested), a "Regenerate backup codes" action in admin Security settings, and a tappable `otpauth://` enrolment deep-link in both the setup gate and settings. **QR *bitmap* deliberately deferred:** no vetted QR dependency is vendored, and I can't scan-verify a hand-rolled QR in-sandbox — so rather than ship an unverifiable image I added the tappable deep-link + manual secret and am flagging the bitmap as future work.
- `EmptyState`/`ErrorState` contrast fix (danger-400 + medium weight); org fleet tabs renamed to the canonical Asset/Operator terms internally (labels were already correct).

**Verification.** api: `tsc` clean, 223/223 unit specs pass. admin + DriverOS: `tsc -b` clean, builds succeed, DriverOS 35/35 vitest + the new AuthProvider spec pass. Website + api e2e/migration paths are validated in CI (real Postgres), which I note rather than claim to have run in this sandbox.
...
**Round 3 audit remediation.** Third independent audit (`fleethq-platform @ 2b6e2ec`) treated as source of truth; scoped Critical → High → Medium → Low across all four repos.

- **C2 — CI gate before production deploy (DevOps, Critical).** `deploy-api.yml` no longer triggers on raw push; it now triggers on **`api-ci` completing successfully** (`workflow_run`, `conclusion == 'success'`) and checks out the exact validated `head_sha`, so a commit whose lint/typecheck/migrate/build/tests failed can't reach Railway. Added `.github/CODEOWNERS`; documented enabling required-status-check + required-review branch protection on `main`. Surfaced in `README.md` and both go-live runbooks (was only in the internal SDLC audit doc).
- **C3 — Bootstrap fails loud (DevOps, Critical).** `prod-bootstrap.ts` now imports `../instrument` (Sentry) and, on a genuinely-broken outcome (empty permission catalog, admin catalog not reconciled, a requested admin account that couldn't be created), reports to Sentry and `process.exit(1)`. `docker-entrypoint.sh` dropped the `|| echo … non-fatal` swallow, so `set -e` aborts the boot — the app only starts on a cleanly-bootstrapped DB, meaning `/health/ready` green now implies bootstrap succeeded, and Railway keeps the previous version live on failure.
- **C1 — Fabricated-infrastructure docs (Compliance, Critical, carried forward wider).** The same present-tense "already built" Terraform/RDS/KMS/CloudFront pattern found in **8 further docs** neither prior pass audited. Added a planned/target-architecture banner to each, reworded the explicit false claims (`PRODUCT_ROADMAP.md`'s "✅ built" backup/DR checkmark → "📋 Planned"; `security-model.md`'s at-rest encryption claim → planned, with the real Integration-Hub AES-256-GCM vault called out as the only at-rest encryption that exists today), and fixed the same pattern in the `instrument.ts` code comment. Added `.github/workflows/docs-integrity.yml` — a CI guard that fails if any doc under `docs/`/`FleetOS-Playbook/` references `infra/terraform` without a planned-architecture qualifier, so the pattern can't silently recur.
- **H1/H3 — Staff-console MFA enforcement + forced password rotation (Security, High).** New `AdminPermissionGuard` obligations gate: an admin with `mustResetPassword` set, or without MFA enrolled where required (`ENFORCE_STAFF_ADMIN_MFA`, default on), is blocked from every route except the setup-exempt ones (`me`/`change-password`/`sessions`/`logout`/`mfa/setup`/`mfa/enable`) with `403 ADMIN_SETUP_REQUIRED`. New `POST /v1/admin/auth/change-password` (verify current, strength-check, clear the flag, bump `tokenVersion`, re-mint the caller's token). Bootstrap now creates the company admin with `adminMustChangePassword` and the staff admin with `mustResetPassword` — the temporary credential typed into deploy config can't outlive first sign-in.
- **H2 — Bootstrap creds documented (DevOps, High).** `BOOTSTRAP_STAFF_ADMIN*` + `ENFORCE_STAFF_ADMIN_MFA` documented in `api/.env.example` with a rotate-after-use warning matching `BOOTSTRAP_COMPANY_ADMIN`, and referenced in the runbooks.
- **H8 — Client-supplied sort/order (API, High — the Round 2 claim that had no code).** Now real: `ListQueryDto` gains `sort`/`order` with `resolveOrderBy(allowedColumns, fallback)` — injection-safe (the client selects from a per-service allowlist, never supplies a column name). Adopted on the assets and operators list endpoints.
- **Medium (backend).** finalize-invoice idempotency key (`admin-billing.service`); `ParseUUIDPipe` across the whole admin-billing controller + the admin-auth `sessions/:id` route; `assertOwnedRecord` companion helper finishing the ownership-check consolidation in the checklists/forms **read** paths; bare `submittedAt`/`createdAt` indexes for the cross-tenant admin browse (migration `20260803010000`); `reconcileAdminPermissions` now prunes stale grants on the two system-template roles (grant/revoke symmetry, template roles only); bootstrap-created accounts now write an admin audit record.
- **Frontend / DriverOS / website.** Tracked in each repo's own history — admin app responsive drawer + WCAG-AA Badge + dashboard/billing error states + CommandPalette a11y + Asset/Operator labels; DriverOS 401-mid-drain queue preservation, `queryClient.clear()` + push teardown on logout, awaited wipe; marketing-site "encrypted at rest" tile regression corrected.
- **Documented deferrals (spec Section 10 — roadmap, not defects).** Scoped API keys / OAuth client-credentials, outbound webhooks wired to real domain events, generic CSV export for core registries, scheduled/emailed reports, an SMS channel, self-issued GST tax-invoice PDFs, and a public OpenAPI/Swagger developer portal remain unbuilt and are honestly marked as future work rather than faked.
...
- **Production enablement — staff admin at boot.** The "issue a customer login" feature lives in the FleetHQ staff console (`/admin`), a separate auth realm from the customer app, so an operator needs an `AdminUser` to reach it — but the only creator of the first staff admin was `scripts/bootstrap-admin.ts`, a `ts-node` script excluded from the runtime image and unable to run in the deployed container. `prod-bootstrap.ts` now (a) reconciles the admin-permission catalog + Super Admin / Support system roles on every boot (idempotent), and (b) creates the first staff Super Admin from env when `BOOTSTRAP_STAFF_ADMIN=true` (username/email/password/full-name fall back to the customer `BOOTSTRAP_ADMIN_*` values, so a solo operator can use one set of credentials for both apps). Idempotent: an existing staff username is left untouched. The reconcile logic moved to `src/bootstrap/admin-permissions-reconcile.ts` so it compiles into `dist/`; `prisma/reconcile-admin-permissions.ts` is now a thin CLI re-export, keeping the `ts-node` callers' import paths unchanged.
...
- **High/Medium backend batches (platform).** Security: SSRF guard now pins the validated IP into the outbound connection (Node core `lookup`, no new dep) closing the DNS-rebinding TOCTOU, plus NAT64 (`64:ff9b::/96`) decoding; `seed-load-test-data.ts` now calls `assertSafeToSeed`; `POST /v1/users` returns the same neutral `USER_NOT_FOUND` as `link` (no username-enumeration oracle); admin MFA-challenge JWT pinned to HS256. Auth: WebAuthn passkey registration now requires step-up re-auth + audit + owner notification + other-session revocation; first-time OAuth auto-link notifies the owner; WebAuthn challenges are single-use (`WebauthnChallengeConsumption`). Billing: Stripe Billing Portal restricted to the checkout price allowlist via a managed Portal Configuration; idempotency keys on every AdminBillingService Stripe write; plan-limit check-then-insert now runs SERIALIZABLE with retry (TOCTOU); webhook coverage for `trial_will_end`/`subscription.paused`/`charge.dispute.created`; native trial-ending reminder sweep; GST `automatic_tax` on admin manual invoices; `stripe_webhook_events` retention purge. Admin backend/API-hygiene: `ParseUUIDPipe` on all admin UUID params; `MAX_AGGREGATION_ROWS` caps on admin-support + predictive-maintenance + graph `findMany`; CSV formula-injection neutralised in the audit export; impersonation now writes a marker into the tenant's own timeline; admin-created users write a TimelineEvent; cross-tenant find-customer-by-email; a full admin-users management console (the permission catalog's `admin_users:*`). DB/code-quality: `ComplianceDocument.operator_id` FK → RESTRICT; `FeatureFlagOverride` companyId index; a stateless `assertOwnership` helper applied to checklists/forms/attached-units/privacy; client-supplied sort/order on the shared list DTO.
...
- **K — Code quality**: customer/depot import natural-key de-duplication (partial-unique indexes + `importByName` with a P2002 race-safe fallback, also covering the Integration Sync path); `MAX_AGGREGATION_ROWS` caps on the remaining unbounded growth-table `findMany` (warehouse stock/machine logs, auth sessions + password-history, predictive maintenance); the Batch-H ownership helper extended with an `{ allowArchived }` option and adopted by Maintenance/Messages/Jobs (ComplianceService deliberately left as-is — `AssetsService`/`OperatorsService` depend on it, so injecting either back would be a circular dependency); and a pure-logic unit-test layer (checklist evaluation, barcode matching, report aggregation — 24 tests). Verified: tsc/eslint clean, 24 unit + 133 e2e green.
- **Marketing site (`FleetHQWebsite`)**: corrected the audit's #1 finding — removed the present-tense SSO/SCIM and 99.9%-SLA claims (SSO/SCIM reframed as on the roadmap) and softened the data-residency / encryption-at-rest / "data centre in Melbourne" claims to what is true today (Australian-hosted, encrypted in transit, database-enforced least privilege via row-level security). Genuine company-location references kept. Verified: typecheck/lint/tests (29/29)/`next build` clean.

**Deliberately not changed (with reasons):**

- **SSO/SAML/OIDC and SCIM** (as built product features) — net-new features (roadmap), not defects; the marketing site now presents them honestly as roadmap.
- **Legal documents** (Privacy Policy, ToS, DPA, SLA on the marketing site) — left as honest "being finalised" placeholders; publishing the reviewed drafts as binding legal terms is a founder/legal sign-off step, deliberately not taken here.
