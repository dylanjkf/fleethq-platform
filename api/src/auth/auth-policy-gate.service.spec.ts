...
describe('AuthPolicyGateService', () => {
  ...
  it('lets a login through when the company has no security policy at all', () => {
    ...
    const result = gate.checkPolicy({ id: 'u1', mfaEnabledAt: null, passwordChangedAt: new Date() }, noPolicyMembership, {});
    expect(result).toBeNull();
  });

  it('blocks with mfa_setup_required when MFA is mandated but not enabled', () => {
    const result = gate.checkPolicy({ id: 'u1', mfaEnabledAt: null, passwordChangedAt: new Date() }, membership(true), { loginMethod: 'password' });
  ...
  it('lets a login through when MFA is mandated and already enabled', () => {
    const result = gate.checkPolicy({ id: 'u1', mfaEnabledAt: new Date(), passwordChangedAt: new Date() }, membership(true), { loginMethod: 'password' });
    expect(result).toBeNull();
  });

  it('a verified WebAuthn/passkey login satisfies a mandatory-MFA policy on its own', () => {
    const result = gate.checkPolicy({ id: 'u1', mfaEnabledAt: null, passwordChangedAt: new Date() }, membership(true), { loginMethod: 'webauthn' });
    expect(result).toBeNull();
  });
  ...
  it('blocks with password_expired once the password has aged past the policy window', () => {
    ...
    const result = gate.checkPolicy({ id: 'u1', mfaEnabledAt: null, passwordChangedAt: staleDate }, membership(false, 30), {});
  ...
  it('lets a login through when the password is still within the expiry window', () => {
    ...
    const result = gate.checkPolicy({ id: 'u1', mfaEnabledAt: null, passwordChangedAt: recentDate }, membership(false, 30), {});
    expect(result).toBeNull();
  });

  it('checks MFA before password expiry — a login missing both gets mfa_setup_required first', () => {
    const staleDate = new Date(Date.now() - 40 * 24 * 60 * 60 * 1000);
    const result = gate.checkPolicy({ id: 'u1', mfaEnabledAt: null, passwordChangedAt: staleDate }, membership(true, 30), { loginMethod: 'password' });
    expect(result).toEqual({ status: 'mfa_setup_required', setupToken: expect.any(String) });
  });
