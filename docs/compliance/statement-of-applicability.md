...
cloud-hosted (AWS) multi-tenant SaaS with a remote team and **no physical
premises**, so most Annex A **7 (Physical)** controls are either inherited from
AWS under the shared-responsibility model (AWS data centres are themselves ISO
27001 / SOC 2 certified) or not applicable.

**Status legend:** ✅ Implemented · 🟡 Partial · 📝 Planned (documented, owner/action outstanding) · ☁️ Inherited (AWS) · ➖ Not applicable.

Evidence references point at this repository unless noted. Cross-references:
[SEC] = `docs/security/cyber-essentials/`, [DB] = `docs/database/`,
...
| 5.7 Threat intelligence | 🟡 | Dependabot advisories + CodeQL + GitHub security advisories in CI [SEC 04/06] |
| 5.8 Information security in project management | 🟡 | Secure-SDLC gates in CI [SEC 06]; formalise in project checklist |
| 5.9 Inventory of information & associated assets | 🟡 | Data model is the information inventory [DB]; infra inventoried as IaC; formal asset register planned |
...
| 5.14 Information transfer | ✅ | TLS everywhere; forced DB TLS; no unencrypted transfer channels [SEC 01/10] |
...
| 5.21 Security in the ICT supply chain | ✅ | Dependency + action pinning; Dependabot; SCA gate; gitleaks [SEC 04/06] |
| 5.22 Monitoring/review of supplier services | 📝 | Periodic sub-processor review to be scheduled |
| 5.23 Information security for use of cloud services | ✅ | AWS shared-responsibility; IaC-defined, tfsec-scanned infra [SEC 01, terraform-ci] |
...
| 5.28 Collection of evidence | ✅ | Append-only `audit_logs` (no UPDATE/DELETE grant) + CloudWatch [SEC 09] |
| 5.29 Information security during disruption | 🟡 | Multi-AZ, cross-region snapshot copy [SEC backup-DR]; DR runbook ownership *TBD* |
| 5.30 ICT readiness for business continuity | 🟡 | Backup/DR design in place; restore drill outstanding [RR] |
...
| 5.33 Protection of records | ✅ | Append-only audit log; RDS PITR + versioned S3 [SEC 09, backup-DR] |
...
| 7.14 Secure disposal or re-use of equipment | ☁️ | AWS media destruction; S3/RDS KMS crypto-erase [SEC 10] |
...
| 8.6 Capacity management | ✅ | Sized connection pooling; autoscaling ECS; scale test [DB] |
| 8.7 Protection against malware | 🟡 | Upload type/magic-byte validation + size caps [SEC 05]; server-side AV scan planned [RR] |
| 8.8 Management of technical vulnerabilities | ✅ | `npm audit` gate + Dependabot + CodeQL + weekly scan [SEC 04] |
| 8.9 Configuration management | ✅ | IaC (Terraform) + tfsec; fail-fast env validation [SEC 02] |
...
| 8.13 Information backup | ✅ | RDS automated backups + PITR + cross-region snapshot copy [SEC backup-DR] |
| 8.14 Redundancy of processing facilities | ✅ | Multi-AZ RDS + ECS; CloudFront [SEC 01] |
| 8.15 Logging | ✅ | Append-only `audit_logs`; structured pino logs with request ids [SEC 09] |
| 8.16 Monitoring activities | 🟡 | Infra alarms + audit trail; security-signal alerting planned [SEC 09, RR] |
| 8.17 Clock synchronization | ☁️ | AWS NTP on ECS/RDS hosts |
| 8.18 Use of privileged utility programs | ✅ | No ad-hoc prod DB access by app role; migrations via a separate role/job |
| 8.19 Installation of software on operational systems | ✅ | Immutable container images from CI; ECR; no in-place installs |
| 8.20 Networks security | ✅ | Segmented VPC, security groups, no public DB [SEC 01] |
| 8.21 Security of network services | ✅ | ALB/CloudFront TLS policies; forced DB TLS [SEC 01] |
| 8.22 Segregation of networks | ✅ | Public/private subnets; chained security groups [SEC 01] |
| 8.23 Web filtering | 🟡 | CloudFront + planned WAF (egress N/A for a SaaS backend) [RR] |
| 8.24 Use of cryptography | ✅ | KMS at rest; TLS in transit; bcrypt; [POL] §Cryptography |
| 8.25 Secure development life cycle | ✅ | CI gates: lint, typecheck, tests, SAST, SCA, secret scan [SEC 06] |
...
| 8.28 Secure coding | ✅ | oxlint/eslint, CodeQL security queries, parameterised queries (Prisma) [SEC 06] |
| 8.29 Security testing in development & acceptance | ✅ | Tenant-isolation + auth + MFA + audit e2e; SAST; scale test [SEC 11] |
| 8.30 Outsourced development | ➖ | Development is in-house |
| 8.31 Separation of dev/test/production | ✅ | Per-environment tfvars + isolated state; test DB throwaway [SEC 02] |
| 8.32 Change management | ✅ | Version-controlled migrations; PR + CI; manual-approval prod deploy [DB migrations] |
...
Of the 93 Annex A controls: the **technological (A.8)** and access/logging/crypto
controls are overwhelmingly **Implemented**, physical controls are **inherited
from AWS or N/A**, and the **Planned/Partial** items are concentrated in
**organizational and people controls** (A.5/A.6) — signed policies, HR processes,
named owners, and the independent review — which are the governance work that
[readiness.md](./readiness.md) sequences toward certification.
