...
| R3 | No alerting on security signals — an attack in progress goes unnoticed | Append-only audit log + infra alarms (data exists, not evaluated) | Med | High | **Med** | Mitigate | Wire CloudWatch metric filters on audit/failed-login → SNS; set Sentry DSN · *Eng* |
...
| R6 | Disaster recovery not proven at scale | RDS PITR + cross-region snapshot copy + multi-AZ (configured, untested) | Low | High | **Med** | Mitigate | Timed restore drill; record actual RPO/RTO · *Eng/Ops TBD* |
| R7 | ~~No edge WAF~~ — **mitigated**: WAFv2 (AWS managed common + known-bad-inputs + per-IP rate rule) on ALB + CloudFront | WAFv2 web ACLs; @nestjs/throttler; segmented network | Low | Med | **Low** | Mitigate | Done (infra/terraform/environments/base/waf.tf); apply on next deploy · *Eng* |
| R8 | Supply-chain vulnerability in dependencies | `npm audit` gate + Dependabot + CodeQL + gitleaks + tfsec | Med | Med | **Low** | Mitigate | Maintain; add remediation-SLA policy · *Eng* |
