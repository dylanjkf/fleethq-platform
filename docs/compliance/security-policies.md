...
- **In transit:** TLS 1.2+ everywhere — client↔edge, edge↔API, and API↔database
  (`rds.force_ssl` + `sslmode=require`). HSTS with a 2-year max-age + preload.
- **At rest:** AWS KMS (customer-managed key, rotation enabled) for RDS and S3.
- **Secrets:** sourced from AWS Secrets Manager at runtime; never in images,
  task definitions, or the repository. Boot fails if a dev-only credential is
...
- All changes go through **pull request + code review** with required, green CI
  before merge *(branch protection to be enabled — R14)*.
- CI enforces: lint, typecheck, tests, **SAST (CodeQL)**, **dependency scanning
  (`npm audit` + Dependabot)**, **secret scanning (gitleaks)**, and **IaC
  scanning (tfsec)**.
...
- Primary sub-processors: **AWS** (hosting), **Stripe** (billing), **AWS SES**
  (email). Each is a reputable provider with its own ISO 27001 / SOC 2
  attestations.
